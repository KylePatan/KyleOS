...
/// Schema version 30 — V0.9 Reports increment 6 (PRD §13.9): Stand-Up Reports' "Jokes moved to
/// New/Done" requires Joke/Chunk status history, deliberately deferred when `HistoryEvent` was
/// first built in V29 (see that schema file's own doc comment — this is the increment it was
/// deferred to). `HistoryEvent` gains two more plain optional subject relationships, `joke`/
/// `chunk`, same additive shape as the original `workItem`/`clip`/`sketchProject` trio — still
/// "exactly one of several," just five options now instead of three, still enforced by the
/// service layer, not the schema.
///
/// `Joke`/`Chunk` each gain a cascade-owned `historyEvents: [HistoryEvent]` array, same shape as
/// `WorkItem.historyEvents`/`Clip.historyEvents`.
///
/// All other models re-declared unchanged from V29 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV30: VersionedSchema {
