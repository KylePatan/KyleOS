...
enum KyleOSMigrationPlan: SchemaMigrationPlan {
    static var schemas: [any VersionedSchema.Type] {
        [KyleOSSchemaV1.self, KyleOSSchemaV2.self, KyleOSSchemaV3.self, KyleOSSchemaV4.self, KyleOSSchemaV5.self, KyleOSSchemaV6.self, KyleOSSchemaV7.self, KyleOSSchemaV8.self, KyleOSSchemaV9.self, KyleOSSchemaV10.self, KyleOSSchemaV11.self, KyleOSSchemaV12.self, KyleOSSchemaV13.self, KyleOSSchemaV14.self, KyleOSSchemaV15.self, KyleOSSchemaV16.self, KyleOSSchemaV17.self, KyleOSSchemaV18.self, KyleOSSchemaV19.self, KyleOSSchemaV20.self, KyleOSSchemaV21.self, KyleOSSchemaV22.self, KyleOSSchemaV23.self, KyleOSSchemaV24.self, KyleOSSchemaV25.self, KyleOSSchemaV26.self, KyleOSSchemaV27.self, KyleOSSchemaV28.self, KyleOSSchemaV29.self, KyleOSSchemaV30.self, KyleOSSchemaV31.self, KyleOSSchemaV32.self, KyleOSSchemaV33.self, KyleOSSchemaV34.self, KyleOSSchemaV35.self, KyleOSSchemaV36.self]
    }

    static var stages: [MigrationStage] {
        [migrateV1toV2, migrateV2toV3, migrateV3toV4, migrateV4toV5, migrateV5toV6, migrateV6toV7, migrateV7toV8, migrateV8toV9, migrateV9toV10, migrateV10toV11, migrateV11toV12, migrateV12toV13, migrateV13toV14, migrateV14toV15, migrateV15toV16, migrateV16toV17, migrateV17toV18, migrateV18toV19, migrateV19toV20, migrateV20toV21, migrateV21toV22, migrateV22toV23, migrateV23toV24, migrateV24toV25, migrateV25toV26, migrateV26toV27, migrateV27toV28, migrateV28toV29, migrateV29toV30, migrateV30toV31, migrateV31toV32, migrateV32toV33, migrateV33toV34, migrateV34toV35, migrateV35toV36]
    }
    ...
    /// V20 adds Source and Clip (new models, with new relationships from Clip to Source/Joke/
    /// Chunk and a new plain field on FileReference) — all new models/relationship arrays/plain
    /// optional fields, the same safe additive shape as every stage except V8.
    static let migrateV19toV20 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV19.self,
        toVersion: KyleOSSchemaV20.self
    )

    /// V21 adds a new plain optional relationship on WorkItem (clip) plus its inverse array on
    /// Clip — genuinely Optional/additive, the safe shape, not V8's mistake.
    static let migrateV20toV21 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV20.self,
        toVersion: KyleOSSchemaV21.self
    )

    /// V22 adds SketchProduction (a new model, with a new relationship from Project to it) —
    /// the same safe additive shape as every stage except V8.
    static let migrateV21toV22 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV21.self,
        toVersion: KyleOSSchemaV22.self
    )

    /// V23 adds FilmShoot (a new model, with new relationships from SketchProduction and
    /// CalendarEvent to it) — the same safe additive shape as every stage except V8. Unlike V8,
    /// FilmShoot's own scalar fields (already-populated-model risk) don't apply here — FilmShoot
    /// itself is the new model, not a field bolted onto the already-populated SketchProduction.
    static let migrateV22toV23 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV22.self,
        toVersion: KyleOSSchemaV23.self
    )

    /// V24 adds CallSheet (a new model, with a new relationship from FilmShoot to it) — the same
    /// safe additive shape as V23, for the same reason (CallSheet's own scalar fields aren't a
    /// migration risk; only the new relationship on the already-populated FilmShoot is, and
    /// that's a plain optional).
    ...
    /// V26 adds CapacityOverride — a brand new top-level model with no relationships to any
    /// existing model, the same safe additive shape as V25's DayJobOverride.
    static let migrateV25toV26 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV25.self,
        toVersion: KyleOSSchemaV26.self
    )

    /// V27 adds PostingItem (a new model) plus two new plain optional cascade relationships on
    /// already-populated models (Project.postingItem, Clip.postingItem) — the same safe additive
    /// shape as V16's Gig/CalendarEvent addition (new model + new optional relationship on an
    /// existing model, not a new required scalar).
    static let migrateV26toV27 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV26.self,
        toVersion: KyleOSSchemaV27.self
    )

    /// V28 adds AppSettings.postsPerWeekTarget — a new Optional scalar on an already-populated
    /// model, the safe V8-lesson shape (not a `= literal` default on a non-optional type).
    static let migrateV27toV28 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV27.self,
        toVersion: KyleOSSchemaV28.self
    )

    /// V29 adds HistoryEvent (a new model) plus three new plain optional cascade relationships on
    /// already-populated models (WorkItem.historyEvents, Clip.historyEvents,
    /// Project.historyEvents) — the same safe additive shape as V27's PostingItem addition.
    static let migrateV28toV29 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV28.self,
        toVersion: KyleOSSchemaV29.self
    )

    /// V30 adds two more plain optional relationships to the already-populated HistoryEvent model
    /// (joke, chunk) plus two new plain optional cascade relationships on already-populated
    /// models (Joke.historyEvents, Chunk.historyEvents) — the same safe additive shape as V29's
    /// own addition.
    static let migrateV29toV30 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV29.self,
        toVersion: KyleOSSchemaV30.self
    )

    /// V31 adds AppSettings.weekendCreativeCapacityHours — a new Optional scalar on an
    /// already-populated model, the safe V8-lesson shape, same as V28's own addition.
    static let migrateV30toV31 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV30.self,
        toVersion: KyleOSSchemaV31.self
    )

    /// V32 adds the new Packet/PacketItem models plus a new `packetItems` relationship array on
    /// the already-populated Document/Project models — additive only, same lightweight shape as
    /// every other new-model migration in this plan.
    static let migrateV31toV32 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV31.self,
        toVersion: KyleOSSchemaV32.self
    )

    /// V33 adds Source.isArchived/archivedAt — a new Optional scalar on an already-populated
    /// model, the safe V8-lesson shape, same as Project's own archive fields.
    static let migrateV32toV33 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV32.self,
        toVersion: KyleOSSchemaV33.self
    )

    /// V34 adds SketchProduction.isReel (a new Optional scalar) and the new Project.reelClip <->
    /// Clip.sketchProject relationship pair — both additive, same safe lightweight shape.
    static let migrateV33toV34 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV33.self,
        toVersion: KyleOSSchemaV34.self
    )

    /// V35 adds the new Submission model plus a plain optional `WorkItem.submission`
    /// back-reference on the already-populated WorkItem model — additive only, same lightweight
    /// shape as every other new-model migration in this plan (Packet/PacketItem at V32 most
    /// recently).
    static let migrateV34toV35 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV34.self,
        toVersion: KyleOSSchemaV35.self
    )

    /// V36 adds the new Folder model plus plain optional `Source.folder`/`Clip.folder`
    /// back-references on the already-populated Source/Clip models — additive only, same
    /// lightweight shape as every other new-model migration in this plan.
    static let migrateV35toV36 = MigrationStage.lightweight(
        fromVersion: KyleOSSchemaV35.self,
        toVersion: KyleOSSchemaV36.self
    )
}
