...
/// Schema version 9 — V0.2 Writing's second increment: the High-Level Act Outline (PRD §6.11/
/// §14.4). A new Act model gives structured, reorderable story architecture — customizable,
/// renameable, reorderable, addable, deletable, never forced to exactly three — belonging to an
/// Act Outline Document. This is a new model + a new relationship array on an existing model
/// (Document.acts), the same safe additive shape proven across V4-V7 — NOT a new required
/// scalar on an existing model, which is what broke lightweight migration in V8 (see that
/// schema's `Project.status` doc comment). All other models re-declared unchanged from V8 (see
/// the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV9: VersionedSchema {
    static var versionIdentifier = Schema.Version(9, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
