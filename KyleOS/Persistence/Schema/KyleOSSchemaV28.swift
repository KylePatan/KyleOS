...
/// Schema version 28 — V0.8 Post It increment 2 (PRD §8.8/§10.4): Posting Cadence. "The user
/// should be able to set a general goal such as 2–3 or 3 posts per week. Kyle OS should suggest
/// open posting dates and avoid unnecessary clustering when possible."
///
/// New `AppSettings.postsPerWeekTarget: Int?` — genuinely Optional (the V8-lesson-safe shape for
/// a new scalar on an already-populated model), with `displayPostsPerWeekTarget` supplying the
/// PRD's own "3 posts per week" example as the fallback for pre-existing rows. No new model this
/// stage — `PostingCadenceService`'s suggestion algorithm is a pure function over already-stored
/// confirmed post dates, needing nowhere new to persist.
///
/// All other models re-declared unchanged from V27 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV28: VersionedSchema {
