...
/// Schema version 29 — V0.9 Reports increment 3 (PRD §14.19/§13.5): Status/Progress History.
/// "Important status changes and progress changes should create history records with timestamps.
/// This enables turnaround and progress-over-time reporting."
///
/// New `HistoryEvent` model (kind: statusChanged/progressChanged, oldValue/newValue as Strings,
/// occurredAt) attaches to exactly one of `WorkItem`, `Clip`, or `Project` (Sketch status lives on
/// `Project.status`, not a separate model) — the same "exactly one of several plain optional
/// back-references, enforced by the service layer" shape as `PostingItem`'s clip/sketchProject.
/// Values are stored as Strings (each subject's status enum has a different Swift type — `String`
/// is the simplest common representation, not a designed-in flexibility feature) rather than
/// three parallel typed history models, avoiding tripling the model/service surface for what's
/// fundamentally the same log shape.
///
/// Scoped to WorkItem/Clip/Project this stage — Joke/Chunk status history (PRD §13.9's "Jokes
/// moved to New/Done") is deliberately deferred to whenever a Stand-Up-Reports-specific increment
/// actually needs it, same "narrowest real slice" discipline as every other module this session.
///
/// `historyEvents` arrays on `WorkItem`/`Clip`/`Project` are cascade-owned, the same shape as
/// `WorkItem.workSessions`/`Clip.postingItem` — a history log doesn't outlive the thing it's a
/// log of, consistent with how `WorkSession`/`PlannedSession` already cascade with their WorkItem
/// rather than surviving independently.
///
/// All other models re-declared unchanged from V28 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV29: VersionedSchema {
