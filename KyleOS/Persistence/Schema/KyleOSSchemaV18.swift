...
/// Schema version 18 — V0.3 Stand Up's sixth increment: After-Gig Notes (PRD §7.10). "After a
/// gig, Kyle OS may optionally ask how the set went. Notes can be attached to the gig,
/// individual Jokes, or Chunks. Performance history should remain lightweight in V1." Two new
/// Optional String fields, both the safe shape (not V8's mistake): `Gig.afterGigNotes` (the
/// overall "how'd it go" reflection, distinct from `Gig.notes`'s pre-gig planning notes) and
/// `GigSetListItem.performanceNotes` (a note on how *this specific* Joke/Chunk landed at *this
/// specific* gig — GigSetListItem already ties a piece of material to one gig's performance, so
/// it's the natural home for a per-performance note, more precise than writing into the Joke/
/// Chunk's own standing `notes` field, which isn't gig-specific and would be overwritten by the
/// next gig's reflection). "Performance history remains lightweight" — no separate history/
/// analytics model, just these two capture fields. All other models re-declared unchanged from
/// V17 (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV18: VersionedSchema {
