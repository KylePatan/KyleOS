...
/// Schema version 23 — V0.5 Sketches' second increment: Film Scheduling (PRD §9.3). "Schedule
/// Film Date should capture: Date, Call time, Estimated wrap, Location, Address, Cast, Crew,
/// Wardrobe, Props, Equipment notes, Parking/access instructions, General notes. The shoot
/// automatically appears on Calendar and generally acts as a hard calendar commitment once
/// cast/crew/location are involved."
///
/// New `FilmShoot` model, not bolted onto the already-populated `SketchProduction` from V22 —
/// SketchProduction has real rows the moment increment 1 ships, so any *new* non-optional scalar
/// field added directly to it would hit the V8 lesson (lightweight migration doesn't reliably
/// apply a stored property's `= literal` default to already-populated rows). A brand-new model
/// has no such problem: `FilmShoot`'s own fields use plain non-optional defaults exactly like
/// every other model's `init`, and `SketchProduction.filmShoot: FilmShoot?` — the only new field
/// on the already-populated model — is a plain optional relationship, the proven-safe shape used
/// for every such addition this session (`WorkItem.joke`/`.chunk`/`.clip`, `Chunk.headlineSet`,
/// etc.). "Date" and "Call time" fold into one `callTime: Date` (a Date value already carries
/// the day), the same simplification `Gig.startAt` already uses.
///
/// Cascade-owned by its SketchProduction (same shape as `Gig.calendarEvent` — a shoot record has
/// no meaning without its production). Auto-creates/keeps in sync a real CalendarEvent of the
/// `.filmShoot` type (existed unused since V4, waiting for exactly this) via the same
/// `SketchProductionService` sync helper every scheduling/logistics update calls — "generally
/// acts as a hard calendar commitment once cast/crew/location are involved" becomes
/// `isHardCommitment = true` whenever any of `location`/`cast`/`crew` is non-empty, recomputed on
/// every relevant edit. `CalendarEvent` gains a plain `filmShoot: FilmShoot?` field, same shape
/// as its existing `gig`/`deadline`/`project`/`workItem` fields.
///
/// All other models re-declared unchanged from V22 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV23: VersionedSchema {
    static var versionIdentifier = Schema.Version(23, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
