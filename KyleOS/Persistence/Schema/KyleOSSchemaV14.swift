...
/// Schema version 14 — V0.3 Stand Up's second increment: the Joke -> Chunk hierarchy (PRD
/// §7.5-§7.6). New Chunk model ("a larger thematic/story run of related jokes") + a new
/// relationship array on Chunk pointing at Joke, nullify not cascade — "removing a Joke from a
/// Chunk should only remove the relationship, not delete the Joke" (§7.6), so deleting a Chunk
/// must not delete its member Jokes either. Joke (an existing, already-populated model since
/// V13) gains two new fields: `chunk: Chunk?` (a plain optional relationship — the same
/// already-proven-safe shape as `WorkItem.document`/`Project.lastOpenedDocument`, no default-
/// value complication) and `orderWithinChunk: Int?` (genuinely Optional, not a `= literal`
/// default on a non-optional Int — the V8 lesson: that shape is what breaks lightweight
/// migration, a true Optional doesn't). All other models re-declared unchanged from V13 (see the
/// note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV14: VersionedSchema {
    static var versionIdentifier = Schema.Version(14, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
