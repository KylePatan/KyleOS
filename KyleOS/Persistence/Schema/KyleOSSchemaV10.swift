...
/// Schema version 10 — V0.2 Writing's third increment: the Scene Outline (PRD §6.11/§14.4). A
/// new Scene model belongs to an Act (not to a separate "Scene Outline" Document) — see this
/// increment's own reasoning in SceneService.swift's doc comment for why. Same safe additive
/// shape as V9 (new model + new relationship array on an existing model), not a repeat of V8's
/// mistake (a new required scalar on an existing model). All other models re-declared unchanged
/// from V9 (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV10: VersionedSchema {
