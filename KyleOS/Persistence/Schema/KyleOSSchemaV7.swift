...
/// Schema version 7 — adds FileReference (PRD §16.4/§14.15/§ "Local File References"): a
/// pointer to an external file (large video, etc.) via a macOS bookmark rather than a copy.
/// Project gains a nullify-owned collection. All other models are re-declared unchanged from V6
/// (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV7: VersionedSchema {
    static var versionIdentifier = Schema.Version(7, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
    ...
    final class Project {
        ...
        /// New in V7. Nullify, not cascade — a FileReference points at potentially irreplaceable
        /// external media (raw footage); deleting the organizing Project must not destroy the
        /// reference or its bookmark, only the link (CLAUDE.md §5: data safety over polish).
