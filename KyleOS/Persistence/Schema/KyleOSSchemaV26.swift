...
/// Schema version 26 — V0.6 Calendar increment 4: Daily Creative Capacity overrides (PRD §11.6).
/// "The user can override a day's expected Creative Capacity, including setting it to zero or
/// increasing it for a free day."
///
/// New `CapacityOverride` model (date + hours) is a brand-new top-level model with no
/// relationships to any existing model — the same simplest-possible additive shape as
/// DayJobOverride (V25) and Joke (V13). No fields were added to any already-populated model this
/// stage. `CreativeCapacityService.todaysCapacity` (unchanged in shape, just gained an `overrides`
/// parameter) uses the override's `hours` directly as that day's baseline when one exists,
/// bypassing the normal Settings/gig-night calculation entirely — an explicit override is the
/// user's own stated number, not an input to adjust further.
///
/// All other models re-declared unchanged from V25 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV26: VersionedSchema {
    static var versionIdentifier = Schema.Version(26, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self, CapacityOverride.self]
    }
