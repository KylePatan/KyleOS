...
/// Schema version 36 — Folders (Kyle, 2026-09-02): "I should be able to put things in folders so
/// I know what things are what. Like Mac file system, I can highlight multiple in clips and
/// 'Create folder' and put items in it. I also should be able to drag sources and clips and
/// whatever i'm doing, and drag and drop them to organize myself." New `Folder` model (title/kind)
/// plus plain optional `Source.folder`/`Clip.folder` back-references — the same "new model + a
/// new optional relationship on an already-populated model" shape every prior new-module
/// migration in this schema already used (Packet/PacketItem at V32, Submission at V35). `kind`
/// scopes a Folder to exactly one content type (Sources or Clips) rather than allowing an
/// ambiguous mixed folder — see `FolderService.swift`.
///
/// All other models re-declared unchanged from V35 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV36: VersionedSchema {
    static var versionIdentifier = Schema.Version(36, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self, CapacityOverride.self, PostingItem.self, HistoryEvent.self, Packet.self, PacketItem.self, Submission.self, Folder.self]
    }
    ...
    final class Project {
        ...
        /// New in V32 (Kyle, 2026-08-19): a whole Project (Writing or Sketch) can be curated
        /// directly into a submission Packet. Cascade so deleting a Project cleans up any
        /// PacketItem that pointed at it rather than leaving a dangling entry — the packet itself
        /// survives, just with one fewer item.
        @Relationship(deleteRule: .cascade, inverse: \PacketItem.project)
        var packetItems: [PacketItem] = []
        ...
        /// New in V34 (Kyle, 2026-08-20): "when [REEL is] selected, it then goes into the clips
        /// folder as well so that we can start editing/subtitling/post it." The Clip that carries
        /// a script-less Reel's actual post-production tracking. Nullify, not cascade — a Clip
        /// represents real editing/session history (same "outlives its origin" reasoning as
        /// `Clip.workItems`), so deleting the Project must only detach it, never destroy it.
        @Relationship(deleteRule: .nullify, inverse: \Clip.sketchProject)
        var reelClip: Clip?
    ...
    final class SketchProduction {
        ...
        /// New in V34 (Kyle, 2026-08-20): "sometimes we'll film a really quick reel/sketch that
        /// doesn't have a script... let's have an option... for 'REEL.'" Optional, V8-lesson-safe.
        var isReel: Bool?
        var displayIsReel: Bool { isReel ?? false }
    ...
    final class Document {
        ...
        /// New in V32 (Kyle, 2026-08-19): a Document (Draft/Prose/Script) can be curated directly
        /// into a submission Packet, independent of whether its parent Project is also in that
        /// packet. Cascade so deleting a Document cleans up any PacketItem that pointed at it.
        @Relationship(deleteRule: .cascade, inverse: \PacketItem.document)
        var packetItems: [PacketItem] = []
    ...
    enum Workspace: String, Codable, CaseIterable {
        ...
        /// New in V35 — Kyle's own "SUBMISSION" quick-add kind (festival/production-company
        /// submission deadlines). See `SubmissionService.swift`.
    ...
    final class WorkItem {
        ...
        var clip: Clip?
        /// New in V35. Inverse of `Submission.workItems` — same plain-optional shape as
        /// `joke`/`chunk`/`clip` above, distinguishing the "Submission" entry task from the
        /// "Submission Reminder" yearly-reopening nudge purely by `workTypeName`, same pattern
        /// `WorkItemService.clipWorkItem`/`clipPostingWorkItem` already use for Clip's own
        /// multiple per-stage WorkItems.
    ...
    enum CalendarEventType: String, Codable, CaseIterable {
        ...
        /// New in V35 — same "give it its own label on Calendar" treatment `.postDate` already
        /// got (Kyle, 2026-08-17), so a submission due date reads clearly next to ordinary
        /// deadlines rather than as a generic Hard Deadline.
        case submissionDeadline = "Submission Deadline"
    ...
    final class Source {
        ...
        /// Optional, the V8-lesson-safe shape for a new scalar on an already-populated model — same
        /// as `Project.isArchived`/`archivedAt`, which this mirrors exactly.
        var isArchived: Bool?
        var archivedAt: Date?
        var displayIsArchived: Bool { isArchived ?? false }

        /// New in V36 (Kyle: "I should be able to put things in folders... drag sources and
        /// clips... to organize myself"). Plain optional, nullify on the Folder side — a Folder
        /// is purely organizational, deleting one must never delete the Sources inside it.
        var folder: Folder?
    ...
    final class Clip {
        ...
        /// New in V34 — the inverse side of `Project.reelClip` (see that property's own doc
        /// comment). Set when this Clip is a Sketch Project's Reel, so its editing/subtitling/
        /// posting can be tracked right here on the normal Clips board.
        var sketchProject: Project?

        /// New in V36 — same shape/reasoning as `Source.folder`.
        var folder: Folder?
    ...
    /// New in V32 (Kyle, 2026-08-19): "I want to add another section. PACKET... Create a Packet,
    /// name the packet of the show/production company I want to send it to, and from there I can
    /// pick projects/drafts/sketches/prose into the packet." A named, freestanding collection
    /// (not itself a Project — a Packet curates *references* into existing Projects/Documents, it
    /// doesn't own or duplicate their content).
    @Model
    final class Packet {
        @Attribute(.unique) var id: UUID
        var title: String
        /// The show or production company this packet targets — e.g. "Saturday Night Live,"
        /// "This Hour Has 22 Minutes." Kyle's own phrasing distinguishes this from `title` (the
        /// packet's own name), so kept as a separate field rather than overloading one string.
        var targetName: String
        var createdAt: Date
        var updatedAt: Date

        @Relationship(deleteRule: .cascade, inverse: \PacketItem.packet)
    ...
    /// New in V32. One curated reference inside a Packet — either a whole Project ("projects" /
    /// "sketches" in Kyle's own wording, both are `Project` rows, distinguished by
    /// `projectType`) or a specific Document ("drafts" / "prose", both `Document` rows —
    /// `documentType == .prose` is Kyle's own "series of jokes but NOT stand up" case). Exactly
    /// one of `project`/`document` is set per item, never both — enforced by
    /// `PacketService.addProject`/`addDocument`, not by a schema-level constraint (SwiftData has
    /// no CHECK-constraint equivalent), same trust-the-domain-layer approach used elsewhere in
    /// this schema (e.g. WorkItem's several optional owner references).
    @Model
    final class PacketItem {
        @Attribute(.unique) var id: UUID
        var packet: Packet?
        var project: Project?
        var document: Document?
    ...
    /// New in V35 (Kyle, 2026-08-27): "SUBMISSION... festivals we need to submit for - have due
    /// dates... sections for SUBMISSION COMING UP and SUBMISSION ENTERED. Also it's a yearly
    /// thing, but the date may change." Not tied to a Project/Document — a Submission tracks a
    /// standalone target (a festival, a production company's open call) that recurs year over
    /// year, so `title` names the target itself, not a piece of writing. `status` is the whole
    /// binary Kyle asked for ("these are binary, we need a way to complete them") — `.comingUp`
    /// vs `.entered` is exactly the app's own working state, kept separate from the two WorkItems'
    /// own `WorkItemStatus` (one WorkItem per role — "Submission" the entry task, "Submission
    /// Reminder" the yearly reopening nudge — both distinguished purely by `workTypeName`, see
    /// `Submission.workItems` below and `SubmissionService.swift`).
    @Model
    final class Submission {
        ...
        /// Nullify, not cascade — same reasoning as `Joke.workItems`/`Chunk.workItems`/`Clip.
        /// workItems`: a WorkItem is real session/time-tracking history that must outlive the
        /// content it was about, even though in practice `SubmissionService.delete` deletes the
        /// Submission and its WorkItems together (Kyle never asked for a Submission to survive
        /// its own deletion the way, say, a Deadline's CalendarEvent does).
        @Relationship(deleteRule: .nullify, inverse: \WorkItem.submission)
        var workItems: [WorkItem] = []
        ...
        init(
            ...
            title: String,
            dueAt: Date? = nil,
            status: SubmissionStatus = .comingUp,
            notes: String = "",
            createdAt: Date = .now
        ) {
            self.id = id
    ...
    /// New in V36 (Kyle, 2026-09-02): "I should be able to put things in folders so I know what
    /// things are what. Like Mac file system, I can highlight multiple in clips and 'Create
    /// folder' and put items in it. I also should be able to drag sources and clips and whatever
    /// i'm doing, and drag and drop them to organize myself." A flat (no nesting — not asked for,
    /// and nothing currently needs it) organizational container. `kind` scopes a Folder to
    /// exactly one content type — a "Sources" folder can only ever hold Sources, a "Clips" folder
    /// only Clips — so the two module's own folder lists (`FolderService.folders(ofKind:in:)`)
    /// never have to sift out the other kind's rows, and a folder never ends up in the confusing
    /// mixed state of holding both.
    @Model
    final class Folder {
        @Attribute(.unique) var id: UUID
        var title: String
        ...
        /// Nullify, not cascade — deleting a Folder is purely an organizational undo; the Sources/
        /// Clips inside it must survive, same "container doesn't own its contents' existence"
        /// reasoning as `Packet.items` (though that one cascades the *reference* row, not the
        /// content itself — here there's no intermediate item row at all, the content models
        /// point at their Folder directly, so nullify is the whole story).
        @Relationship(deleteRule: .nullify, inverse: \Source.folder)
        var sources: [Source] = []

        @Relationship(deleteRule: .nullify, inverse: \Clip.folder)
        var clips: [Clip] = []
