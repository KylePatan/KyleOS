...
/// Schema version 11 — V0.2 Writing's sixth increment: Workspace Restoration (PRD §6.17),
/// scoped to its most self-contained, valuable part — "last open document" — since cursor
/// location/scroll position need editor internals that don't exist yet (SwiftUI's plain
/// TextEditor doesn't expose them) and split-view state/panel widths have no split view to
/// restore yet (§6.12/§6.13, a later increment). Project gains a plain, unidirectional optional
/// reference to a Document — the same already-proven-safe shape as `WorkItem.document` (no
/// `@Relationship` wrapper needed for a to-one reference with no inverse collection to maintain).
/// All other models re-declared unchanged from V10 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV11: VersionedSchema {
    static var versionIdentifier = Schema.Version(11, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
