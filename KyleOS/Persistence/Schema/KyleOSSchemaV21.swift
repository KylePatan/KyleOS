...
/// Schema version 21 — V0.4 Clips' second increment: Editing progress/timer, wiring the
/// universal Focus Timer into Clips (its own separate `docs/DEVELOPMENT_ROADMAP.md` V0.4
/// bullet, mirroring how Stand-Up Work Sessions — PRD §7.11 — was Stand Up's own later
/// increment). `WorkItem` gains a plain optional `clip: Clip?` relationship, the same proven-
/// safe shape as `WorkItem.joke`/`WorkItem.chunk`; `Clip` gains the matching inverse array
/// (`@Relationship(deleteRule: .nullify, inverse: \WorkItem.clip) var workItems: [WorkItem] =
/// []`) — nullify, not cascade: deleting a Clip must not erase its logged Work Session history,
/// only detach it (same reasoning as `Joke.workItems`/`Chunk.workItems`). Unlike Stand-Up, Clips
/// has no "general" untargeted session in the PRD — every Clips timer is against a specific
/// Clip, so there's no `generalClipsWorkItem` counterpart to `generalStandUpWorkItem`. All other
/// models re-declared unchanged from V20 (see the note at the top of KyleOSSchemaV2.swift for
/// why this pattern matters).
enum KyleOSSchemaV21: VersionedSchema {
