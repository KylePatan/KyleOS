...
/// Schema version 13 — V0.3 Stand Up's first increment: the Joke Board (PRD §7.2-§7.4). A Joke
/// is a standalone top-level object, deliberately NOT nested under Project — unlike Writing's
/// discrete TV Pilot/Screenplay projects, Stand Up material is one continuous, ongoing joke
/// library, matching how the PRD never mentions a Project relationship for Jokes (§14.11). New
/// Joke model — a brand new top-level model, the simplest possible additive shape (no existing
/// model even needs a new relationship array for this one). All other models re-declared
/// unchanged from V12 (see the note at the top of KyleOSSchemaV2.swift for why this pattern
/// matters).
enum KyleOSSchemaV13: VersionedSchema {
    static var versionIdentifier = Schema.Version(13, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
