...
/// Schema version 35 — Submissions (Kyle, 2026-08-27): "There needs to be another thing we can
/// put in for 'HOME'... SUBMISSION. And then it creates a home task. These are binary... We should
/// also add a section for submissions. Here we will have things like festivals we need to submit
/// for - have due dates... and also have sections for (SUBMISSION COMING UP) and (SUBMISSION
/// ENTERED). Also it's a yearly thing, but the date may change. So if I submit to a festival it
/// should remind me a month before that the submissions may be coming up again." New `Submission`
/// model (title/dueAt/status/notes) plus a plain optional `WorkItem.submission` back-reference —
/// the same "new model + one new optional relationship on an already-populated model" shape every
/// prior new-module migration in this schema already used (Packet/PacketItem at V32, most
/// recently). New `Workspace.submissions` case and `CalendarEventType.submissionDeadline` case
/// round it out — see `SubmissionService.swift` for the actual create/enter/reopen behavior,
/// including how the "remind me a month before" yearly nudge is computed.
///
/// All other models re-declared unchanged from V34 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV35: VersionedSchema {
    static var versionIdentifier = Schema.Version(35, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self, CapacityOverride.self, PostingItem.self, HistoryEvent.self, Packet.self, PacketItem.self, Submission.self]
    }
