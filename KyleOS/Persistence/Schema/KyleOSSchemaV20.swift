...
/// Schema version 20 — V0.4 Clips' first increment: Source Media, Child Clips, and the
/// Production status board (PRD §8.1-§8.4). "Clips is a content-production pipeline that turns
/// source footage into ready-to-post social content": Source Footage -> Identify Clip -> Isolate
/// -> Edit -> Subtitle if needed -> Ready -> Post.
///
/// New `Source` model (§8.2: title/recordingDate/location/notes + a file reference to the local
/// video file — "Kyle OS should normally reference the existing local video file instead of
/// copying it," reusing the existing `FileReference`/bookmark infrastructure rather than
/// building a second one). `FileReference` gains a plain `source: Source?` field (same shape as
/// its existing `project: Project?`); `Source.fileReference` is cascade (the bookmark has no
/// independent existence apart from its Source, same reasoning as `Gig.calendarEvent`).
///
/// New `Clip` model (§8.3: title/description/source-timestamps/notes/editing-notes/status/
/// progress/post-date, plus an optional Joke/Chunk cross-reference — "related Joke/Chunk
/// reference," a stand-up clip's footage often maps to a specific bit). Cascade-owned by its
/// Source (a Clip has no meaning without one, same shape as `Document.acts`); references Joke/
/// Chunk by nullify (same "reference existing material" shape as every prior cross-reference in
/// this codebase). `ClipStatus` is the PRD's own 7 "core states" (§8.4) — kept as the real,
/// full-fidelity stored status rather than the board's simplified 5-lane display grouping, which
/// is a view-layer concern (`ClipBoardView`'s own doc comment covers the exact lane mapping, a
/// documented simplification per CLAUDE.md §13, not a data-model decision).
///
/// Deliberately NOT in this increment (later Clips increments, per `docs/DEVELOPMENT_ROADMAP.md`
/// V0.4's own bullet list): Editing progress/timer (its own separate roadmap bullet, mirrors how
/// Stand-Up Work Sessions was Stand Up's own later increment), the Ready Content Buffer/
/// Production Backlog/Scheduled Posts distinction (§8.7), and Post Date backward-scheduling
/// (§8.5/§8.6 — explicitly V0.7 Scheduling Engine territory per CLAUDE.md §10/§2, not decided
/// here). `postDate` itself is stored on Clip this increment (plain field, no scheduling
/// automation) since PRD roadmap explicitly lists "Post Dates" under V0.4.
///
/// All other models re-declared unchanged from V19 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV20: VersionedSchema {
    static var versionIdentifier = Schema.Version(20, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
