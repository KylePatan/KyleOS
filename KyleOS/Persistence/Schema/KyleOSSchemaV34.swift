...
/// Schema version 34 — Reels (Kyle, 2026-08-20): "sometimes we'll film a really quick reel/sketch
/// that doesn't have a script... let's have an option to have a sketch project started - and when
/// it is opened there can be an option for 'REEL' and when that's selected, it then goes into the
/// clips folder as well so that we can start editing/subtitling/post it." Two additive pieces:
/// `SketchProduction.isReel` (a new Optional scalar, the V8-lesson-safe shape) marks a Sketch
/// Project as script-less, and a new `Project.reelClip` <-> `Clip.sketchProject` relationship pair
/// links that Project to the real `Clip` that carries its actual editing/subtitling/post-date
/// tracking on the normal Clips board — the same infrastructure every other Clip already uses,
/// not a second copy of it inside Sketches.
///
/// All other models re-declared unchanged from V33 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV34: VersionedSchema {
    static var versionIdentifier = Schema.Version(34, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self, CapacityOverride.self, PostingItem.self, HistoryEvent.self, Packet.self, PacketItem.self]
    }
