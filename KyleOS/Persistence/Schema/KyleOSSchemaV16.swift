...
/// Schema version 16 — V0.3 Stand Up's fourth increment: Gigs (PRD §7.8). "Stand Up should
/// store gigs with date, venue, show, start time, set length, location, and notes. Gigs
/// automatically appear on Calendar." New `Gig` model, plus a one-to-one owned relationship to
/// the existing `CalendarEvent` model: creating/editing a Gig creates/keeps in sync a real
/// `CalendarEvent` of type `.standUpGig` (a type that already existed since V4 and is already
/// read by `CreativeCapacityService`'s stand-up-night bonus — PRD §4.4's exact wording, "a
/// stand-up gig night generally supports about 1 additional creative hour," which is what's
/// actually implemented; §7.8's own "reduce expected Creative Capacity" line is inconsistent
/// PRD wording that this increment deliberately does not act on, since the already-shipped and
/// tested §4.4 behavior is the more specific, decision-gate-free spec — see GigService's doc
/// comment). Gig -> CalendarEvent is cascade (deleting a Gig deletes its auto-created event; the
/// event has no independent existence), the same shape as `Project.deadline`. CalendarEvent
/// gains a plain `var gig: Gig?` inverse field, same shape as its existing `project`/`workItem`/
/// `deadline` fields. All other models re-declared unchanged from V15 (see the note at the top
/// of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV16: VersionedSchema {
    static var versionIdentifier = Schema.Version(16, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
