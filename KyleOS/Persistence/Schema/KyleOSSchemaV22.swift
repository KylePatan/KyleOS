...
/// Schema version 22 — V0.5 Sketches' first increment: Writing Handoff and the production
/// status board (PRD §9.1/§9.2). "Sketches begins when a Sketch is marked Writing Finished...
/// A finished Sketch should appear in Sketches automatically as the same Project, not a copied
/// project." No new "handoff" relationship or action is needed for this — `WritingProjectType.
/// sketch` and `ProjectStatus.finished` already exist (since V8), so a finished Sketch project
/// is identified by those two existing fields; `SketchProductionService.finishedSketchProjects`
/// just filters for them.
///
/// New `SketchProduction` model holds the parts of PRD §9 that don't belong on the generic
/// `Project` (which Writing also uses for five other project types): production `status` (the
/// PRD's own pipeline, minus "Written" — that's `ProjectStatus.finished`, not a SketchProduction
/// state — "Filming Not Scheduled -> Filming Scheduled -> Filmed -> Editing -> Ready -> Posted")
/// and `postDate` (§9.8, storage only — no backward-scheduling automation, same V0.7-deferred
/// reasoning as Clips' `postDate`). Cascade-owned by its Project (same shape as `Gig.
/// calendarEvent` — a production record has no meaning without its Project); lazily created on
/// first status/post-date change, not eagerly for every Sketch project, so merely *viewing* the
/// board never writes anything (`SketchProductionService.status(for:)` falls back to
/// `.filmingNotScheduled` when no record exists yet, the same Optional-with-computed-fallback
/// shape used throughout this codebase).
///
/// Deliberately NOT in this increment (later Sketches increments, per `docs/
/// DEVELOPMENT_ROADMAP.md` V0.5's own bullet list): Film Scheduling's full field set and its
/// auto-created CalendarEvent (§9.3 — the `.filmShoot` CalendarEventType already exists, unused
/// since V4, waiting for this), Call Sheet generation/export (§9.4/§9.5), and Editing's Focus
/// Timer wiring (§9.7, mirrors Clips' own later timer increment). All other models re-declared
/// unchanged from V21 (see the note at the top of KyleOSSchemaV2.swift for why this pattern
/// matters).
enum KyleOSSchemaV22: VersionedSchema {
    static var versionIdentifier = Schema.Version(22, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
