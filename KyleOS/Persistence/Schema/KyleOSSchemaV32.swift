...
/// Schema version 32 — Packet (PRD-unspecified, Kyle 2026-08-19): "I want to add another section.
/// PACKET... Create a Packet, name the packet of the show/production company I want to send it
/// to, and from there I can pick projects/drafts/sketches/prose into the packet." New `Packet` and
/// `PacketItem` models, plus a new `packetItems` cascade-inverse relationship on the already-
/// populated `Document` and `Project` models (so deleting a Document/Project cleans up any
/// PacketItem that referenced it, instead of leaving a dangling packet entry) — the same safe
/// additive shape as every other new-relationship-array migration in this file's history (Acts,
/// ScriptBlocks, Drafts, etc.), not a new non-optional scalar, so no V8-lesson risk.
///
/// All other models re-declared unchanged from V31 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV32: VersionedSchema {
