...
/// Schema version 27 — V0.8 Post It increment 1 (PRD §10/§14.18). "Clips and Sketches should
/// share a Posting Item model where practical. It stores related content, suggested/confirmed
/// post date, ready state, posting status, platform information, and actual posted date. Home's
/// Post It view should query these items instead of maintaining another list."
///
/// New `PostingItem` model deliberately does NOT store its own ready/posted status — that would
/// duplicate `Clip.status`/`SketchProduction`'s status (via `Project`) as a second source of
/// truth with a real drift risk (the Ready Queue's existing `ClipStatus.ready`/`.posted` cases
/// already fully answer "is this ready/posted"). Instead `PostingItemService.displayStatus`
/// computes the PRD §10.2 five-state display (Not Ready/Ready/Due Today/Posted/Overdue) fresh
/// from the content's own ready/posted booleans plus this model's `confirmedPostDate` — the same
/// "computed property over stored flag" choice already used for `GigService.needsAfterGigNotes`.
/// `PostingItem` itself only stores what's genuinely new: `suggestedPostDate`, `confirmedPostDate`
/// (distinct per §8.8 — suggested can move, confirmed is a hard deadline), `platform` (free text —
/// PRD doesn't specify an enum of platforms), and `actualPostedDate` (set when the user marks the
/// content Posted, distinct from the planned `confirmedPostDate`).
///
/// `Project.postingItem`/`Clip.postingItem` are cascade-owned single relationships, the same
/// proven-safe shape as `Project.sketchProduction`/`Gig.calendarEvent` — a Post It entry has no
/// meaning without the content it schedules. `PostingItem` itself references back via plain
/// optional `clip`/`sketchProject` fields (exactly one set, enforced by `PostingItemService`, same
/// "exactly one of X/Y" pattern as `GigSetListItem`'s joke/chunk).
///
/// All other models re-declared unchanged from V26 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV27: VersionedSchema {
    static var versionIdentifier = Schema.Version(27, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self, CapacityOverride.self, PostingItem.self]
    }
