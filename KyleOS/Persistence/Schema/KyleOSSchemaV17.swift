...
/// Schema version 17 — V0.3 Stand Up's fifth increment: Gig Set Lists (PRD §7.9). "A Gig may
/// have a planned set list built from existing Chunks and Jokes. Set-list items reference
/// existing material rather than duplicate it." New `GigSetListItem` model — a thin ordered
/// join referencing exactly one of `joke`/`chunk` (never both), owned by its `Gig` (cascade:
/// deleting a Gig deletes its set-list rows, which have no meaning without it) but referencing
/// Joke/Chunk by nullify (same "reference existing material" shape as every other cross-
/// reference in this codebase — `Joke.chunk`, `Chunk.headlineSet`, `CalendarEvent.gig`; deleting
/// the referenced Joke/Chunk elsewhere must not delete the set-list row, only null out the
/// reference, per the Fourth lesson: an undeclared inverse leaves a dangling reference instead
/// of nullifying). `Joke`/`Chunk` each gain a new `setListAppearances: [GigSetListItem]`
/// relationship array — the safe additive shape (new relationship array with `= []` default),
/// not V8's mistake (a non-optional scalar). All other models re-declared unchanged from V16
/// (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV17: VersionedSchema {
    static var versionIdentifier = Schema.Version(17, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
