...
/// Schema version 15 — V0.3 Stand Up's third increment: the Headline Set (PRD §7.7). "The
/// Headline Set should primarily be built from ordered Chunks" — scoped to just Chunks this
/// increment (the PRD's own "primarily"); "loose jokes may also be allowed when useful" is a
/// documented later refinement, not decided here (CLAUDE.md §13). New HeadlineSet model + a
/// relationship array pointing at the existing Chunk model, nullify not cascade — removing a
/// Chunk from a Headline Set, or deleting the Headline Set, must not delete the Chunk (same
/// data-safety reasoning as V14's Chunk->Joke relationship). Chunk (already-populated since V14)
/// gains two new fields: `headlineSet: HeadlineSet?` (plain optional relationship, the
/// established safe shape) and `orderInHeadlineSet: Int?` (genuinely Optional, not a `=
/// literal` default — the V8 lesson's safe shape). All other models re-declared unchanged from
/// V14 (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV15: VersionedSchema {
    static var versionIdentifier = Schema.Version(15, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self]
    }
