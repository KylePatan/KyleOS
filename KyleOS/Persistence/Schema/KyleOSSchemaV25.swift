...
/// Schema version 25 — V0.6 Calendar increment 3: Day-Job blocks (PRD §11.4). "Monday–Friday,
/// 8 AM–5 PM is blocked by default. Individual days can be overridden for vacation or unusual
/// availability." Day-Job blocks are materialized as ordinary `.dayJob` CalendarEvents (generated
/// lazily by `DayJobScheduleService` as the Calendar workspace views each month), so they're
/// editable/deletable through the same UI as any other event. "Overridden" means: deleting a
/// generated block for a specific day must not silently regenerate it — that requires a durable
/// record of the override, which a CalendarEvent's mere absence can't represent (absence is
/// ambiguous between "never generated yet" and "deliberately removed").
///
/// New `DayJobOverride` model (date + isOff) is a brand-new top-level model with no relationships
/// to any existing model — the simplest possible additive shape, same as Joke (V13)'s own
/// introduction. No fields were added to any already-populated model this stage.
///
/// All other models re-declared unchanged from V24 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV25: VersionedSchema {
    static var versionIdentifier = Schema.Version(25, 0, 0)

    static var models: [any PersistentModel.Type] {
        [Project.self, AppSettings.self, WorkTypeDefault.self, Document.self, Draft.self, Act.self, Scene.self, ScriptBlock.self, Joke.self, Chunk.self, HeadlineSet.self, Gig.self, GigSetListItem.self, Source.self, Clip.self, SketchProduction.self, FilmShoot.self, CallSheet.self, WorkItem.self, Deadline.self, CalendarEvent.self, PlannedSession.self, WorkSession.self, ActiveTimerState.self, FileReference.self, DayJobOverride.self]
    }
