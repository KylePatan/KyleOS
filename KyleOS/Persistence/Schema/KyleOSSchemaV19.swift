...
/// Schema version 19 — V0.3 Stand Up's seventh and final increment: Stand-Up Work Sessions (PRD
/// §7.11). "The user can start a timed session against a specific Joke/Chunk or a general
/// Stand-Up Writing session." Unlike Writing (`WorkItemService.writingWorkItem`, keyed off a
/// Document that always belongs to a Project), Stand-Up material deliberately has no Project
/// (see this schema's V13 note on Joke) — so `WorkItem` gains two new plain optional
/// relationships, `joke: Joke?` and `chunk: Chunk?` (a WorkItem for a "general" session has
/// neither set), the same proven-safe shape as `WorkItem.document`. `Joke`/`Chunk` each gain the
/// matching inverse array (`@Relationship(deleteRule: .nullify, inverse: \WorkItem.joke/.chunk)
/// var workItems: [WorkItem] = []`) — nullify, not cascade: deleting a Joke/Chunk must not erase
/// its logged work-session history, only detach it (same reasoning as every other "reference
/// existing material" relationship in this codebase). All other models re-declared unchanged
/// from V18 (see the note at the top of KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV19: VersionedSchema {
