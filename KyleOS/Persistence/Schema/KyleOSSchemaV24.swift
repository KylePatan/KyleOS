...
/// Schema version 24 — V0.5 Sketches' third increment: the Call Sheet (PRD §9.4). "A scheduled
/// shoot should support generating an editable Call Sheet populated from project data. Fields
/// may include project title, date, call time, wrap time, location/address, cast/characters,
/// crew/roles, wardrobe, props, equipment, parking/access, contact information, scene notes, and
/// additional notes."
///
/// New `CallSheet` model, cascade-owned by its `FilmShoot` (same shape as `SketchProduction.
/// filmShoot` — a call sheet has no meaning without its shoot). "Generating an editable Call
/// Sheet populated from project data" means a real snapshot-and-diverge, not a live mirror of
/// `FilmShoot`: `SketchProductionService.generateCallSheet` seeds every overlapping field
/// (location/address/cast/crew/wardrobe/props/equipment/parking/general-notes) from the current
/// FilmShoot values at generation time, but the Call Sheet is then independently editable —
/// tweaking it afterward does not write back to FilmShoot, and rescheduling FilmShoot does not
/// silently rewrite an already-generated Call Sheet. Two fields have no FilmShoot counterpart and
/// start empty: `contactInformation` and `sceneNotes`. `FilmShoot.callSheet: CallSheet?` is the
/// only new field on an already-populated model (FilmShoot has real rows after increment 2) — a
/// plain optional relationship, the proven-safe shape.
///
/// All other models re-declared unchanged from V23 (see the note at the top of
/// KyleOSSchemaV2.swift for why this pattern matters).
enum KyleOSSchemaV24: VersionedSchema {
