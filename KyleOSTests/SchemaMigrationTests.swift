...
final class SchemaMigrationTests: XCTestCase {
    ...
    func testStoreWrittenUnderV29OpensCleanlyAsV30WithDataIntact() throws {
        let storeURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("KyleOSMigrationV29Test-\(UUID().uuidString)")
            .appendingPathComponent("Store.sqlite")
        try FileManager.default.createDirectory(at: storeURL.deletingLastPathComponent(), withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: storeURL.deletingLastPathComponent()) }

        let jokeID: UUID
        // Write a store using ONLY the V29 schema shape — HistoryEvent.joke/chunk don't exist yet.
        do {
            let container = try ModelContainer(
                for: Schema(versionedSchema: KyleOSSchemaV29.self),
                configurations: [ModelConfiguration(url: storeURL)]
            )
            let context = ModelContext(container)
            let joke = KyleOSSchemaV29.Joke(text: "Airline food, but for cats.", order: 0)
            jokeID = joke.id
            context.insert(joke)
            try context.save()
        }
        ...
        do {
            ...
            XCTAssertTrue(joke.historyEvents.isEmpty, "New relationship should default to empty, not crash or fabricate data")

            // The new relationship must actually be usable post-migration, not just present.
